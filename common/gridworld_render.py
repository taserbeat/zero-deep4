            ...
            for x in range(xs):
                ...
                if r != 0 and r is not None:
                    ...
                    if state == self.goal_state:
                        txt = txt + ' (GOAL)'
                    ax.text(x + .1, ys - y - 0.9, txt)  # type: ignore
                ...
                if (v is not None) and state != self.wall_state:
                    if print_value:
                        ...
                        if v.shape[0] > 7:
                            key = 1
                        offset = offsets[key]
                        ax.text(x + offset[0], ys - y + offset[1], "{:12.2f}".format(v[y, x]))  # type: ignore
                ...
                if policy is not None and state != self.wall_state:
                    ...
                    for action in max_actions:
                        ...
                        if state == self.goal_state:
                            continue
                        ax.text(x + 0.45 + offset[0], ys - y - 0.5 + offset[1], arrow)  # type: ignore

                if state == self.wall_state:
                    ax.add_patch(patches.Rectangle((x, ys - y - 1), 1, 1, fc=(0.4, 0.4, 0.4, 1.)))  # type: ignore
        plt.show()
            ...
            for x in range(xs):
                for action in action_space:
                    ...
                    if r != 0 and r is not None:
                        ...
                        if state == self.goal_state:
                            txt = txt + ' (GOAL)'
                        ax.text(x + .05, ys - y - 0.95, txt)  # type: ignore
                    ...
                    if state == self.wall_state:
                        ax.add_patch(patches.Rectangle((tx, ty), 1, 1, fc=(0.4, 0.4, 0.4, 1.)))  # type: ignore
                    elif state in self.goal_state:
                        ax.add_patch(patches.Rectangle((tx, ty), 1, 1, fc=(0., 1., 0., 1.)))  # type: ignore
                    else:

                        tq = q[(state, action)]
                        color_scale = 0.5 + (tq / qmax) / 2  # normalize: 0.0-1.0

                        poly = patches.Polygon(action_map[action], fc=cmap(color_scale))
                        ax.add_patch(poly)  # type: ignore

                        offset = offset_map[action]
                        ax.text(tx + offset[0], ty + offset[1], "{:12.2f}".format(tq))  # type: ignore
        plt.show()
